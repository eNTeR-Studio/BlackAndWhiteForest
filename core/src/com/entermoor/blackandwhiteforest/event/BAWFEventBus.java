...
import com.entermoor.blackandwhiteforest.api.BAWFEventReceiver;
import com.entermoor.blackandwhiteforest.api.IBAWFEventReceiver;
		...
		for (int i = 0; i < methods.length; i++) {
			...
			for (int j = 0; j < annotations.length; j++) {
				if (annotations[j].annotationType().getName().equals(BAWFEventReceiver.class.getName())) {
					isSuccessful = isSuccessful && receiverMethodList.add(methods[i])
							&& receiverInstanceList.add(receiver);
					break;
			...
			for (int j = 0; j < annotations.length; j++) {
				if (annotations[j].annotationType().getName().equals(BAWFEventReceiver.class.getName())) {
					isSuccessful = isSuccessful && receiverMethodList.remove(methods[i])
							&& receiverInstanceList.remove(receiver);
					break;
				} else if (j == annotations.length) {
					isSuccessful = false;
				}
